...
- Board-level import создаёт новую копию через последовательность обычных
  API-команд. Workspace import, merge и destructive restore не реализованы.
- Импорт не переносит исходные ID, авторов, timestamps и старую activity
  history. Комментарии получают текущего автора и новое время.
- Client-side import пока не является одной общей транзакцией. При ошибке
  клиент пытается удалить только новую недособранную доску.
