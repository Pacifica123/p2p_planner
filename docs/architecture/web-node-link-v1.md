...
- Статус: реализовано в `v1.0.0-beta.7`, tombstone transfer расширен в `v1.0.0-beta.8`
- Дата: 2026-08-01
...
1. Оба узла обновлены до beta.8, запущены и доступны друг другу в доверенной
   локальной сети.
...
- архивное состояние;
- глобальные tombstone карточек, но не node-local скрытия;
...
- Полный снимок структуры переносится в момент link. Последующие relay-операции
  beta.8 покрывают карточки, lifecycle удаления и чек-листы; создание новых workspace/досок/колонок,
