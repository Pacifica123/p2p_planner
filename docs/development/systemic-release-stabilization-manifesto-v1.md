...
## 14. Отдельная классификация transport-failures

Сбой внешнего Git-remote, например `PUSH_FAILED` из-за `remote: Internal Server Error`, не должен смешиваться с качеством патча или состоянием продукта. Такой отказ классифицируется как transport-only failure: файлы уже могли примениться, проверки уже могли пройти, локальный commit уже мог быть создан, но публикация не состоялась по внешней причине.

Практическое правило: если отказ случился на этапе push, следующий артефакт должен быть либо безопасным повтором того же содержательного изменения с новым `patchId`, либо отдельным operational-fix для devctl retry/push handling. Это не отменяет Deep Release Autopsy, а добавляет к ней еще одну ось наблюдаемости: разделять product/runtime failures и VCS transport failures.
