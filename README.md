...
- If diagnostics show `node` but `npm` fails while the fingerprint points to `npm.CMD`, use the patched `tools/devbootstrap.py`: it resolves `.cmd`/`.bat` launchers through `cmd.exe /d /s /c` for probes, gates and managed frontend startup. Do not edit `frontend/package-lock.json` until a real `npm ci` run proves a lockfile mismatch.
