...
python tools/devbootstrap.py release-gates --managed-test-db --prepare-deps --install-playwright-browsers
python tools/devbootstrap.py release-gates --managed-test-db --prepare-deps --install-playwright-browsers --include-real-backend-browser
...
Для безопасного DB-writing release review подготовь отдельную test DB по инструкции `docs/dev-bootstrap/release-gates-test-database.md` и запускай real-backend browser gate через `python tools/devbootstrap.py release-gates --managed-test-db --prepare-deps --install-playwright-browsers --include-real-backend-browser`.
